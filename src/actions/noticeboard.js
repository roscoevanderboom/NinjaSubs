...
import { newJobApplicationData } from "../constants/jobPosts";
...
export const newJobPost = (post, dispatch, feedback) => {
  noticeboard
    .doc(`${post.ref}`)
    .set(post)
    .then(() => {
...
export const applyToJobPost = (post, profileData, feedback) => {
  ...
  handleFirebase("add", post, profileData, details).catch((err) => {
    feedback("error", err.message);
  });
};
export const removeJobApplication = (post, profileData, feedback) => {
  ...
  handleFirebase("remove", post, profileData, details).catch((err) => {
    feedback("error", err.message);
  });
};
