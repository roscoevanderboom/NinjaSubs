...
export const startChat = (profileData, sub, history, dispatch) => {
  let newChat = newChatRoom(profileData, sub);
  privateChats
    .doc(`${newChat.room_id}`)
    .set(newChat)
    .then(() => {
...
export const followChat = (id, history, dispatch) => {
  privateChats.doc(`${id}`).onSnapshot(
    function (doc) {
      if (doc.exists) {
...
export const deleteAllChatrooms = (user) => {
  privateChats
    ...
    .then(() => console.log("Chatrooms deleted"))
    .catch(() => console.log("No chatrooms to delete"));
};
