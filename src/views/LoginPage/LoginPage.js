...
export default function LoginPage() {
  ...
  useEffect(() => {
    var cardAnimationTimeout = setTimeout(function () {
      setCardAnimation("");
    }, 700);
    return () => {
      clearTimeout(cardAnimationTimeout);
    };
  }, []);
          ...
          <GridItem xs={12} sm={12} md={4}>
            <Card className={classes[cardAnimaton]}>
              <form onKeyPress={handleSubmit} className={classes.form}>
                ...
                <FormBody
                  props={{ classes, title, data, errors, handleData }}
                />

                <FormFooter
                  props={{ classes, title, handleSubmit, handleTitle }}
                />
              </form>
            </Card>
          </GridItem>
