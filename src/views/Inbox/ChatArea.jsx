...
export default () => {
  ...
  const blockUser = () => {
    if (selectedChat && window.confirm(confirmMessage)) {
      let user_to_block = selectedChat.participants.filter(
        (uid) => uid !== profileData.uid
      );
      handleProfileDataArrays(
        user,
        "arrayUnion",
        "blackList",
        user_to_block[0]
      ...
      <div className={classes.body}>
        <div className="d-flex flex-column align-items-center col-12 col-md-8 col-lg-7 col-xl-5 p-0 m-0">
          <Toolbar className={classes.toolbar}>
            ...
            <div>
              <Tooltip title="Block user" placement="left">
                <IconButton onClick={blockUser} color="inherit" edge="start">
                  <VoiceOverOff />
                </IconButton>
              </Tooltip>
              <IconButton
